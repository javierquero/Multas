...
async function consultarLanus(browser, tipo, valor) {
  if (tipo === 'dni') return { estado: 'no_soportado', infracciones: [], mensaje: 'Lanus no permite consulta por DNI.' };
  ...
  try {
    ...
    const tieneCaptcha = await page.evaluate(() => {
      ...
      return t.includes('captcha') || !!document.querySelector('.g-recaptcha, [data-sitekey]');
    });
    if (tieneCaptcha) return { estado: 'captcha', infracciones: [], mensaje: 'Lanus requiere CAPTCHA — no automatizable.', urlConsulta: `${BASE}/consultas.php?municipio=lanus` };

    const input = await page.waitForSelector('input[name*="dominio"], input[name*="patente"], input[name*="placa"], input[type="text"]', { timeout: 8000 });
    ...
    await page.waitForFunction(() => {
      const t = document.body.innerText.toLowerCase();
      return t.includes('acta') || t.includes('infraccion') || t.includes('multa') || t.includes('no se encontr') || t.includes('no posee') || t.includes('sin infr');
    }, { timeout: 15000 }).catch(() => {});
    ...
    const resultado = await page.evaluate(() => {
      ...
      const sinInf = texto.includes('no se encontr') || texto.includes('sin infr') || texto.includes('no posee') || texto.includes('no hay');
