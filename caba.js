...
async function consultarAvellaneda(browser, tipo, valor) {
  ...
  try {
    ...
    const input = await page.waitForSelector('input[name*="dominio"], input[name*="patente"], input[name*="placa"], input[type="text"]', { timeout: 10000 });
    ...
    await page.waitForFunction(() => {
      ...
      return t.includes('multa') || t.includes('infraccion') || t.includes('acta') || t.includes('no se encontr') || t.includes('sin result') || t.includes('no registra');
    }, { timeout: 15000 }).catch(() => {});
    ...
    const resultado = await page.evaluate(() => {
      ...
      const sinInf = texto.includes('no se encontr') || texto.includes('sin result') || texto.includes('no registra') || texto.includes('no tiene');
